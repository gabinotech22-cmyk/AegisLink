/**
 * Binary layout of a Double Ratchet state inside the key vault (F-1b phase 3,
 * docs/F1B-KEY-VAULT-DESIGN.md). Twin of `cpp/aegis_ratchet.c` (the struct the
 * C core seals) and of `desktop/src/main/crypto/vault/ratchetState.ts`.
 *
 * The state is fixed-size, so its sealed blob is too: the vault seals it under
 * the profile KEK as a blob of type AEGIS_KEY_RATCHET (6), which never loads as
 * a key handle and no key blob ever opens as a state.
 *
 * Who uses this file:
 *   - the Jest stand-in of the vault (`jest/nodeRatchet.ts`), both ways;
 *   - the app, ONLY to encode a pre-F-1b session (raw keys in JSON) once, for
 *     `vault.ratchetImport` (the one-time migration of design doc section 4);
 *   - tests.
 * Pure: no imports, so the interop harness (`test/ratchet-interop.mjs`) can
 * load it with Node's type stripping.
 *
