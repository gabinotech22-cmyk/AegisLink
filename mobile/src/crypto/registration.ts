 ...
 *
 * Mined natively, off the JS thread (`powSha256` in ./sodium, the C core's
 * `aegis_pow_sha256`): on Hermes (no JIT) the JavaScript loop took several
 * seconds per registration at the relay's difficulty 18 and blocked the JS
 * thread between batches; in C it is a fraction of a second.
 *
 * Protocol: the nonce is the first of "00000000", "00000001", ... that works —
 * the same nonce the JavaScript miner returned. The relay re-hashes
 * `nonce + challenge` as a plain string concatenation
 * (server/src/pow/challenge.ts), so the leading zeros are re-hashed identically
 * and its `/^[0-9a-f]{1,32}$/` gate accepts the fixed-width form.
 */
export async function solvePoW(
  ...
  return powSha256(utf8ToBytes(challenge), difficulty);
}
