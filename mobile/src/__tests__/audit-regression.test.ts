...
describe('Honesty of claims — locales never claim a completed third-party audit', () => {
  // Bug: the onboarding footer shipped "AUDITED 2026 Q1" / "AUDITADO 2026 Q1" /
  // "VERIFICATO 2026 Q1" on the very first screen, but no independent audit has
  // happened (it is a Phase 2 goal — docs/ANDROID-LAUNCH-READINESS.md, "Honestidad
  // de claims"). Marketing claims must have verifiable backing; "auditable" and
  // "audit pending" are honest, "audited" is not until the report exists.
  const LOCALES_DIR = path.join(SRC, 'i18n', 'locales');
  const localeFiles: string[] = fs.readdirSync(LOCALES_DIR).filter((f: string) => f.endsWith('.json'));
  ...
  for (const file of localeFiles) {
    it(`${file} contains no past-tense audit claim in any string`, () => {
    ...
    it(`${file} onboarding.footer makes no audit/verification claim`, () => {
