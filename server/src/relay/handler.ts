...
function attachCallSignaling(socket: Socket, me: string, sockets: Map<string, Set<Socket>>) {
  // Sealed-sender forward: routes by `to`, NEVER stamps `from`.
  // The caller's identity is sealed inside the invite ciphertext and recovered
  // only by the callee. Returns the payload it would emit (sans `to`).
  ...
  // invite and recovered only by the callee. Legacy v1 handlers have been
  // removed — no client emits them under the default v2 policy.
  ...
  // ── Plaintext SDP guards ─────────────────────────────────────────────────
  // call:offer and call:sdp:answer with plaintext SDP are REJECTED.
  // All call signaling must use the sealed NaCl box variants (call:invite:v2 / call:answer:v2).
  ...
  socket.on('call:offer', (_raw) => {
    socket.emit('error_msg', { code: 'plaintext_sdp_rejected', for: 'call:offer',
      message: 'Use call:invite:v2 with sealed NaCl box signaling' });
  });

  socket.on('call:sdp:answer', (_raw) => {
    socket.emit('error_msg', { code: 'plaintext_sdp_rejected', for: 'call:sdp:answer',
      message: 'Use call:answer:v2 with sealed NaCl box signaling' });
  });
...
function attachGroupCallSignaling(socket: Socket, me: string, sockets: Map<string, Set<Socket>>) {
  ...
  // and no current client emits it (replaced by group_call:channel heartbeat).
}
