...
import { messageRepo, senderKeyDistRepo, prekeysRepo, pushRepo, devicesRepo, identityRepo, deliveryTokenRepo, workRepo, workChannelRepo, workMessageRepo, workAttachmentRepo, workChannelPermissionRepo, getPermissions, MESSAGE_TTL_MS, type WorkRole, publicChannelRepo, publicChannelPostRepo, publicChannelJoinRepo } from '../db/client.js';
...
export function attachRelay(io: SocketServer) {
  ...
  async function onAuthenticated(socket: Socket, me: string, deviceId: string | undefined, _challenge: Challenge) {
    ...
    // ─── Sealed Public Channels (Phase 1, docs/SEALED-PUBLIC-CHANNELS.md) ──
    // All events gated behind the PUBLIC_CHANNELS feature flag (default OFF).
    // The relay is a blind forwarder — it never sees `from`, `body`, `seqNum`,
    // or `prevHash`. Posts are always encrypted, even for public channels.
...
// ─── Sealed Public Channels socket events (Phase 1) ─────────────────────────
// Feature-gated: when PUBLIC_CHANNELS is off, all handlers emit 'feature_disabled'.
// The relay is a blind forwarder — no `from` field in any pubchannel event.
// Rate-limiting is keyed by delivery token hash (NOT socket identity) to preserve
// sender anonymity — knowing who sent a message is inside the encrypted payload.
