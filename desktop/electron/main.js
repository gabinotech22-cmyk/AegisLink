...
function createWindow() {
  ...
  if (!app.isPackaged) {
    // Dev: try Vite dev server first, fallback to pre-built dist
  ...
  // F12 toggles DevTools in any mode (useful for diagnosing packaged app issues)
    ...
    cb({
      responseHeaders: {
        ...
        'Content-Security-Policy': [
          ...
          "img-src 'self' data:; connect-src 'self' http://localhost:3000 ws://localhost:3000 http://localhost:3001 ws://localhost:3001 " +
          "https://*.aegislink.com wss://*.aegislink.com; object-src 'none'; base-uri 'none';"
        ],
      },
    });
  });
...
ipcMain.handle('panic:wipe', async (e) => {
  assertTrustedSender(e);
  // 1. Close & delete SQLite (+ WAL/SHM)
  try { if (db) { db.close(); db = null; } } catch {}
  const dbPath = path.join(app.getPath('userData'), 'aegislink.db');
  for (const p of [dbPath, `${dbPath}-wal`, `${dbPath}-shm`]) {
    try { fs.rmSync(p, { force: true }); } catch {}
  }
  // 2. Wipe all keytar entries
  try {
