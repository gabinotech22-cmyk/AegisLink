/**
 * F-1b key vault, desktop: the renderer facade (`crypto/sodium/vault.ts`) over
 * the main-process vault (`src/main/crypto/vault/vault.ts`, real sodium-native
 * secure memory) through the in-process IPC bridge. Twin of
 * `mobile/src/crypto/__tests__/vault.test.ts` (same cases, golden rule #5).
 */
import { describe, it, expect, afterEach } from 'vitest';
import tweetnacl from 'tweetnacl';
import { ml_kem768 as nobleMlKem } from '@noble/post-quantum/ml-kem.js';
import { vault, VaultBlobRejectedError, VaultKeyUnavailableError } from '../vault';
...
describe('key vault', () => {
  ...
  it('refuses wrong key types, sizes and slot names', async () => {
    ...
    expect(() => vault.import('self', 'x25519', rand(31))).toThrow(/bad key length/);
    ...
    expect(() => vault.import('self', 'ed25519', badEd)).toThrow(/does not match its seed/);
