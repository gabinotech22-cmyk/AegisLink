 ...
 * key vault in the MAIN process (F-1b phase 3, docs/F1B-KEY-VAULT-DESIGN.md).
 *
 * The state (root, chain and message keys, our DH and ML-KEM pairs) never
 * exists in the renderer: the vault (`src/main/crypto/vault/ratchet.ts`) holds
 * it while it runs a step and hands it back SEALED under the profile's KEK. A
 * `RatchetState` is that sealed blob plus its non-secret view (`info`:
 * counters and public keys) and the session metadata the transport keeps next
 * to it (`x3dhInit`, `createdAtMs`). A renderer XSS can use a session while
 * the app runs, never read its keys.
 *
 * Twin of `mobile/src/crypto/signal/ratchet.ts` (golden rule #5). The
 * algorithm and the wire format are those of the renderer ratchet this
 * replaces (`src/main/crypto/vault/ratchetCore.ts`): old and new clients
 * interoperate, and sessions saved before phase 3 are imported once
 * (`socket/ratchetSerde.ts`).
 *
 ...
 * Enforced inside the vault (main process, `ratchetCore.ts`).
 */
