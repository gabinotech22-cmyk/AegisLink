...
import { vault, type LegacyRatchetState } from '../crypto/sodium/vault';
...
function importLegacyRatchetState(s: Record<string, unknown>, slot: string): RatchetState {
  ...
  const skipped: LegacyRatchetState['skipped'] = [];
  ...
  // The main process encodes and seals it; vault.ratchetImport zeroes the raw
  // copies it was given, and the skipped keys it was not are zeroed here.
  let sealedRatchet: ReturnType<typeof vault.ratchetImport>;
  try {
    sealedRatchet = vault.ratchetImport(slot, raw);
  } finally {
    for (const e of skipped) e.mk.fill(0);
  }
  const { sealed, info } = sealedRatchet;
  return {
    slot,
    sealed,
    info,
