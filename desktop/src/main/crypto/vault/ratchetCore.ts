 ...
 * what the desktop key vault runs in the MAIN process (F-1b phase 3), so no
 * ratchet key ever reaches the renderer. Twin of
 * `mobile/modules/aegis-sodium/jest/ratchetCore.ts` (the reference the mobile
 * C port is tested against). Before phase 3 this ran in the renderer
 * (`renderer/crypto/signal/ratchet.ts`); the algorithm and the bytes on the
 * wire are unchanged. Pure: primitives come in `RatchetPrims`.
 *
