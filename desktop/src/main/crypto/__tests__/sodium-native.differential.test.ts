 ...
 * Twin: `server/src/__tests__/sodium-native.differential.test.ts`. Also pins the
 * backend itself: `native.ts` must stay byte-identical to the relay's copy.
 */
import { describe, it, expect } from 'vitest';
import fs from 'fs';
import path from 'path';
import tweetnacl from 'tweetnacl';
import * as native from '../sodium/native';
...
describe('native backend twins', () => {
  it('desktop main native.ts is byte-identical to the relay copy', () => {
    const here = fs.readFileSync(path.resolve(__dirname, '../sodium/native.ts'), 'utf8');
    const relay = fs.readFileSync(path.resolve(__dirname, '../../../../../server/src/crypto/sodium/native.ts'), 'utf8');
    expect(here).toBe(relay);
  });
});
