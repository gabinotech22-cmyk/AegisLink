/**
 * bridges — the platform-independent bridge core, desktop twin of
 * mobile/src/net/__tests__/torBridges.test.ts, plus the parity guard: the core
 * logic and the built-in list must be byte-identical on both platforms.
 */
...
describe('parity with mobile (golden rule #5)', () => {
  const repo = join(__dirname, '..', '..', '..', '..', '..');
  const body = (p: string): string => {
    const s = readFileSync(join(repo, p), 'utf8').split(String.fromCharCode(13)).join('');
    return s.slice(s.indexOf('export type TorTransport'));
  };
  it('core logic is byte-identical to mobile/src/net/torBridges.ts', () => {
    expect(body('desktop/src/main/tor/bridges.ts')).toBe(body('mobile/src/net/torBridges.ts'));
  });
  it('built-in bridge list is identical to mobile/src/net/builtinBridges.json', () => {
    const read = (p: string): unknown => JSON.parse(readFileSync(join(repo, p), 'utf8'));
    expect(read('desktop/src/main/tor/builtinBridges.json')).toEqual(read('mobile/src/net/builtinBridges.json'));
  });
});
