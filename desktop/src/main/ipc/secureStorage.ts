...
function assertKeyAllowed(key: string): void {
  ...
  const pattern =
    /^aegis\.(?:[a-zA-Z0-9_\-]+\.)?(secretKey\.b64|signSecretKey\.b64|activeProfile|activeSlotId|slotsList|displayName|avatarColor|avatarImage|profileStatus|workDisplayName|workAvatarColor|workAvatarImage|workProfileStatus|panic\.v1|preferences\.v1|polls\.v1|identity\.v1|prekeys\.v1|prekeysPublished|pin\.v1|pin\.salt\.v2|group\.v1|deviceId|scheduled\.desktop\.v1|distribution\.v1|spkSecret\.b64|spkSecret\.\d+|spk\.keyId|pqSpkSecret\.\d+|pqSpk\.keyId|secdiag\.v1|opkIds\.json|opkSecret\.\d+|self\.ratchet\.[0-9A-HJKMNP-TV-Z\-]+)$/
  if (!pattern.test(key)) {
