...
| ID | Sev | Hallazgo | Fix |
|----|-----|----------|-----|
| A-1 | ⏸️ | Rate limits in-memory no compartidos entre instancias | **DIFERIDO** (follow-up de infra): despliegue mono-instancia → no es vuln activa. Migrar a Redis solo al escalar horizontalmente; añade dependencia dura + riesgo en la VM ([[incident_n8n_oom_vm]]). No bundlear en hardening. **Actualización (rama `feat/socketio-redis-adapter-pm2-cluster`): el CÓDIGO ya soporta la activación** — rate-limit distribuido (`rateLimits.ts`/`redisClient.ts`, preexistente), adapter Redis para Socket.IO cross-proceso (`relay/socketIoRedisAdapter.ts`, nuevo) y config PM2 cluster mode opt-in (`ecosystem.config.cjs`, nuevo), todos gateados por `REDIS_URL` sin degradar el despliegue mono-instancia actual. **Sigue sin activarse en la VM real** — falta provisionar Redis, setear `REDIS_URL` en prod, sticky sessions en nginx si se corre >1 instancia, y migrar `deploy/deploy.sh` a `ecosystem.config.cjs` (los 4 son trabajo de infra fuera de esta rama). Detalle completo: `docs/RELAY-HORIZONTAL-SCALING.md`. |
