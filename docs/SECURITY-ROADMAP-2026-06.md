...
| ID | Sev | Hallazgo | Fix |
|----|-----|----------|-----|
| A-1 | ⏸️ | Rate limits in-memory no compartidos entre instancias | **DIFERIDO** (follow-up de infra): despliegue mono-instancia → no es vuln activa. Migrar a Redis solo al escalar horizontalmente; añade dependencia dura + riesgo en la VM ([[incident_n8n_oom_vm]]). No bundlear en hardening. **Actualización (rama `feat/socketio-redis-adapter-pm2-cluster`): el CÓDIGO ya soporta la activación** — rate-limit distribuido (`rateLimits.ts`/`redisClient.ts`, preexistente), adapter Redis para Socket.IO cross-**máquina** (`relay/socketIoRedisAdapter.ts`, nuevo), adapter de cluster nativo (IPC, sin Redis) para N workers en la MISMA máquina (`relay/socketIoClusterAdapter.ts`, nuevo), sticky routing real vía `@socket.io/sticky` + `src/clusterMaster.ts` (nuevo — reemplaza un primer intento con `exec_mode: 'cluster'` de PM2 que resultó incompatible con sticky sessions, corregido en la misma rama antes de mergear) y config PM2 opt-in que supervisa ese router (`ecosystem.config.cjs`, nuevo). Todo gateado por `REDIS_URL`/`cluster.isWorker` sin degradar el despliegue mono-instancia actual. **Sigue sin activarse en la VM real** — falta decidir activación multi-worker, provisionar Redis si se escala a varias VMs, setear `REDIS_URL` en prod, y migrar `deploy/deploy.sh` a `ecosystem.config.cjs` (trabajo de infra fuera de esta rama). Detalle completo: `docs/RELAY-HORIZONTAL-SCALING.md`. |
...
|----|-----|-----|------|------|
| F-1 | ✅ | L | **Núcleo crypto nativo** (timing side-channel runtime) | **Hecho: relay, desktop y mobile corren libsodium nativo** (mobile compila libsodium desde fuente verificada en `mobile/modules/aegis-sodium`; pruebas y estado en `docs/ROADMAP.md` Hito 3; siguiente paso F-1b). Al abrirse, el crypto core corría en **JS puro** sobre Hermes (mobile) / V8 (desktop): `tweetnacl`, `@noble/*`. Constant-time está garantizado a nivel de **fuente** (linaje NaCl/djb + comparaciones XOR-acumuladas + zeroize), pero **no** verificado a través del JIT+GC del motor JS. No es vuln activa (no hay oráculo remoto: el secreto se opera on-device, por el cable solo va ciphertext normalizado §7.2; explotarlo exige co-residencia local ⇒ ya es endpoint compromise). Plan: portar el hot-path (X25519, XSalsa20-Poly1305, Ed25519, HKDF/HMAC) tras un **binding nativo de libsodium** (módulo Expo envolviendo libsodium, o `react-native-quick-crypto`), conservando la capa de composición en TS y la misma interfaz pública. Sustitución de implementación, **no** cambio de protocolo. Documentado en `PROTOCOL.md` §2.1 + §10.5 + §8.3. **Estado y decisiones (spike 2026-09-24, costura única `crypto/sodium`):** `docs/ROADMAP.md` Hito 3. |
| F-2 | 🟡 | L | **UnifiedPush** (quitar dependencia de Google/Apple en push) — Android: conector hecho (2026-09-26, `mobile/plugins/withUnifiedPush.js`; ver `docs/ROADMAP.md`), iOS sin alternativa a APNs | El push wake-up usa FCM/APNs con payload genérico (sin contenido ni identidad de emisor), pero Google/Apple aprenden que *un* device recibió *un* push. Plan: soportar [UnifiedPush](https://unifiedpush.org/) (ntfy/Gotify self-hosted) como transporte de wake-up alternativo, manteniendo FCM/APNs como fallback para quien no corra un distribuidor. Elimina el metadato de "device X despertó en T" del lado de los gigantes. Documentado en README → Known limitations. |
