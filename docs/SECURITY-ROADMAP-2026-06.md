...
| C-2 | 🟧 | Desktop keystore sin segundo factor (DPAPI = usuario de sesión) | **Fase 1 EN CURSO** (`fix/sec-c2-desktop-lock`): lock desktop era un stub (cualquier PIN de 4 dígitos abría) → PIN Argon2id real con salt por-install + constant-time + rate-limit→pánico (paridad mobile). **Fase 2 (ola siguiente):** envolver `getDbKey` SQLCipher con clave derivada del PIN (2º factor real at-rest sobre DPAPI). |
