...
| R25 | **Backup incompleto y pantalla que miente** (mobile). La tarjeta contaba "N mensajes · Media · Grupos · Dispositivos" y el fichero solo llevaba identidad, perfil y contactos parciales: `groups` y `preferences` existían en el tipo y **nadie los escribía ni leía**; los contactos restaurados perdían apodo, relay propio (federación → inalcanzables), caps, fijado/oculto/pendiente/perfil. | Restaurar ≠ recuperar tu cuenta. | ✅ `toBackupContact`/`toBackupGroup`/`restorablePreferences` (ajustes de bloqueo excluidos a propósito); restore escribe grupos y preferencias; la tarjeta lista lo que hay y avisa de que los mensajes no van. Tests `backupPayload.test.ts` (ambas), `Backup.test.tsx`. |
| R26 | **Desktop: restaurar un backup no restauraba nada.** `decryptBackup` devolvía el payload y la pantalla decía "Account restored, N contacts recovered" sin escribir identidad, perfil ni contactos. Además escribía formato **v1 (PBKDF2 100k)** cuando el doc y mobile llevan v3 Argon2id, y no podía leer un v3 del móvil. | Backup de escritorio teatro; sin restauración cruzada móvil↔escritorio. | ✅ `crypto/backup.ts` portado (v3 escribe, v1/v2/v3 lee); restore escribe identidad (`linkDevice`), perfil, contactos, grupos y ajustes. |
