...
| R22 | **"Silenciar siempre" no silenciaba** (mobile) y **un silencio con plazo no caducaba nunca** (desktop). Mobile comprobaba `mutedUntil && mutedUntil > now` — "siempre" guarda `mutedUntil = 0`, falso; desktop comprobaba `muted \|\| …`, con lo que el flag seguía activo tras la hora. | El silencio más usado no funcionaba en móvil; en escritorio no se podía silenciar "1 hora". | ✅ `utils/mute.ts` (idéntico en ambas): `isContactMutedNow` = flag + plazo (0/null = siempre; pasado = caducado). Aplicado en mensajes, llamada de grupo activa y llamada perdida. Tests `mute.test.ts` (ambas) + casos en `contentPolicy` / `push.policy`. |
| R23 | **Los grupos no se podían silenciar** y la lista "Silenciados" de Notificaciones **siempre estaba vacía**: `preferences.mutedChats` (que las notificaciones sí consultaban) no lo escribía nadie. | Sin control de avisos por grupo; pantalla que promete una lista y no la tiene. | ✅ `mutedChats` + `mutedChatsUntil` (1 h / 8 h / siempre) escritos desde la ficha del grupo (mobile y desktop); la lista de Silenciados se deriva de contactos + grupos con desilenciar real. |
