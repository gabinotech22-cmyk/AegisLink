...
| R28 | **Salir de un grupo era solo local** (mobile + desktop). `leaveGroup` borraba en tu dispositivo y no avisaba: los demás te seguían viendo y cifrando para ti, y el siguiente mensaje **recreaba el grupo** en tu móvil. | No se podía salir de un grupo de verdad. | ✅ Carrier `[group:left]` (autenticado por sealed sender; el emisor solo se quita a sí mismo): el admin hace `removeMember` (re-firma, re-key, difunde), los demás lo quitan en local; `preferences.leftGroupIds` impide la recreación salvo re-invitación del admin. Tests `groups.leave.test.ts`, `client.groupLeave.test.ts` (recepción). Doc PROTOCOL §6.1. |
