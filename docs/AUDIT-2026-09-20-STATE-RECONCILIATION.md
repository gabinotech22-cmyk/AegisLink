...
| R24 | **Las preferencias no formaban parte del señuelo** (mobile; desktop por paridad). Bajo el PIN de coacción, Ajustes mostraba las **palabras clave reales**, los IDs de grupos/chats/canales silenciados reales, y cualquier toggle pulsado **sobrescribía** las preferencias del perfil real. | El señuelo filtraba lo que le importa al usuario y podía corromper su configuración. | ✅ `hydrate()` bajo duress enmascara `notifKeywords`, `mutedChats(+Until)`, `mentionsOnlyChats`, `mutedChannels` a defaults en memoria; `set()` no persiste bajo duress; el PIN real re-hidrata. Lock re-hidrata preferencias al entrar y salir del señuelo. Test `preferences.duress.test.ts`. Doc PROTOCOL §7.6. |
