...
| W-8 | MEDIA | ✅ HECHO | Con `/web3` apagado, el resolver de W-2 no habría sido accesible y la desactivación de W-1 no se podría consultar: un arreglo a medias. | El interruptor se elimina (`server/src/index.ts`): `/web3` ya solo expone el resolver, de solo lectura y con `relayLimiter` (`onion: shared`, como el proxy de previews, tras #523). Guarda en `web3Did.test.ts`. |
