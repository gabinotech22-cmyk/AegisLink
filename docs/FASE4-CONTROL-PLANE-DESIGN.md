...
> **Nota de vigencia (2026-09-19, regla de oro doc #8):** doc **histórico** — refleja el estado en que se escribió; no se reescribe. Escrito pre-Slice 4. **Estado actual:** la bifurcación decidida aquí (socket de control-plane por aegisId + socket mailbox aparte autenticado por posesión) está implementada y activa por defecto — `docs/SEALED-SENDER-ARCHITECTURE.md` §5 (Fase 4 ✅), código `mobile/src/socket/mailboxSocket.ts`, `server/src/relay/handler.ts`. Con la federación (F5a, #490) el socket de control-plane también puede vivir en un relay `.onion` propio (`net/homeRelay.ts`). Desde 2026-09-23 el control-plane va **siempre** por Tor (ya no existe el opt-in `routeViaTor`/Orbot que se menciona abajo), en un circuito separado del buzón — `SEALED-SENDER-ARCHITECTURE.md` §6.2.
