...
> **Nota de vigencia (2026-09-19, regla de oro doc #8):** doc **histórico** — refleja el estado en que se escribió; no se reescribe. Escrito pre-implementación (2026-07-12). **Estado actual:** implementado en Android como servicio de wake de llamadas (`mobile/plugins/withCallWakeService.js`, `withCallForegroundService.js`; `docs/ROADMAP.md` §Hito 4); iOS = APNs de alta prioridad (CallKit activo con app viva; wake PushKit/VoIP post-lanzamiento). Desde 1.0.8 el servicio usa `foregroundServiceType=remoteMessaging`, no `dataSync` (Android 15 prohíbe `dataSync` al arrancar el teléfono y lo limita a 6 h al día; ver cabecera de `withCallWakeService.js`). La señalización cross-relay de llamadas (federación F4) y su clase de wake `call` viven en `docs/PROTOCOL.md` §7.1 y `docs/FEDERATION-DESIGN.md`.
