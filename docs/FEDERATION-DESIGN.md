...
| F5b | ⬜ | `feat/federation-relay-screen` | `RelaySettings` (mobile + desktop): estado actual, "usar mi propio relay" (onion v3 + **Verificar** = `GET /relay/info` + `/health` por Tor), consecuencias en claro, PIN/biometría; `relayMigration.ts` (registrar en el nuevo con PoW → prekeys → ntfy → `profile_update { mailboxRelay }` a contactos y grupos → `setHomeRelay` con `previous = {viejo, until: +7 d}` → el pool mantiene el buzón viejo solo-recepción hasta `until` → `DELETE /identity/:id` firmado en el viejo; rollback si falla el registro); "volver al oficial" = lo mismo al revés; receptor de `mailboxRelay` actualiza `contact.relayOnion`; i18n EN/ES/IT | `RelaySettings.test.tsx`, `relayMigration.test.ts` (orden + rollback), `profileUpdate.relay.test.ts` |
