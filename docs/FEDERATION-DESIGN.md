...
| **F3b** | ⬜ | `feat/federation-first-contact` | **Hallazgo de F3:** el primer mensaje a un contacto nuevo va **siempre en v1** (el inner v2 nunca lleva `x3dhInit`) y v1 solo existe por el transporte aegisId, que no cruza relays; además el `mailboxRoot` del contacto solo llega por `profile_update` de una sesión ya establecida → **el primer contacto cross-relay no puede arrancar**. Diseño (modelo SimpleX: la dirección incluye la cola): (1) el enlace/QR **v2 lleva también el `mailboxRoot`** y la clave de firma Ed25519 del dueño (`aegislink://v2/<id>/<pubkey>/<onion>/<root>/<signPub>`); (2) `encryptMessageV2` admite `x3dhInit` en el inner para el primer mensaje, y el inner de primer contacto lleva `signingPublicKeyB64` + `relay` del emisor; (3) el receptor acepta un v2 de emisor desconocido con **TOFU + binding ID↔clave** (mismo nivel que v1 hoy, donde el `from` se resuelve por directorio) y crea el contacto con `relayOnion` + root del emisor; (4) el sender pide el bundle al relay del contacto (ya en F2). Riesgo declarado: quien tenga el enlace conoce la secuencia de buzones (puede dirigir spam, no leer); mitigación en F5: rotar el root al migrar y opción "regenerar enlace". | KAT del enlace v2 ampliado; `messaging.test` (v2 con `x3dhInit` abre y consume el init); test cliente: A y B en relays distintos, A escanea el enlace de B, primer mensaje llega por el pool y B crea el contacto con relay+root |
